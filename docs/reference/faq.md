...
**全局设置**
在连接字符串里设置。每个数据库写法不一样。如果不清楚 可以在https://www.connectionstrings.com/ 上面查找
...
**单个设置**
FreeSqlHelper.Fsql.Insert(list).CommandTimeout(200).xxxxxxxx
FreeSqlHelper.Fsql.Update<Entities.SysAdmin>().CommandTimeout(300).xxxxxx
FreeSqlHelper.Fsql.Select<Entities.SysAdmin>().CommandTimeout(400).xxxxxx
FreeSqlHelper.Fsql.Delete<Entities.SysAdmin>().CommandTimeout(500).xxxxxx
